...
# 修改ResNet50的残差块，添加SpatialAttention注意力机制
...
class Bottleneck(nn.Module):
    """带SpatialAttention注意力机制的ResNet50残差块"""
    expansion = 4
    
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, use_spatial_att=True):
...
# 创建带SpatialAttention注意力机制的ResNet50模型
def resnet50_spatial_attention(pretrained=True, **kwargs):
    """Constructs a ResNet50 model with SpatialAttention attention."""
    # 创建基础ResNet50模型
    model = models.resnet50(pretrained=pretrained)
    
    # 修改层结构，使用带SpatialAttention的残差块
    layers = []
    ...
    # 修改layer1-4，使用带SpatialAttention的残差块
    # layer1不使用SpatialAttention，保持原始结构
    layers.append(model.layer1)
    
    # layer2-4使用SpatialAttention
    inplanes = 256
    block = Bottleneck
...
if __name__ == "__main__":
    ...
    # 加载带SpatialAttention注意力机制的ResNet50模型
    model_ft = resnet50_spatial_attention(pretrained=True)
    ...
    # 保存模型
    torch.save(model_ft.state_dict(), 'resnet50_spatial_attention_finetuned.pth')
    print("模型已保存为: resnet50_spatial_attention_finetuned.pth")